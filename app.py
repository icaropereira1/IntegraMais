...
# --- SIDEBAR: CREDENCIAIS MARKETPLACE ---
client_id = ""
client_secret = ""
merchant_id = ""

if marketplace == "ifood":
...
tab1, tab2, tab3, tab4 = st.tabs(["📥 1. Baixar planilha Marketplace", "📤 2. Atualizar PDVs no Marketplace", "📥 3. Baixar planilha Vuca", "🔍 4. Comparação"])

with tab1:
    st.header(f"Baixar Cardápio Atual ({marketplace.capitalize()})")
    st.write(f"Gere a planilha bloqueada contendo o cardápio atual do {marketplace.capitalize()}.")
    
    if st.button(f"Gerar Planilha {marketplace.capitalize()}"):
        if marketplace == "ifood":
            ...
            st.warning(f"O download de planilha para {marketplace} ainda não foi implementado.")

with tab2:
    st.header(f"Atualizar Códigos PDV ({marketplace.capitalize()})")
    ...
    if arquivo_upload is not None:
        if st.button(f"🚀 Iniciar Atualização no {marketplace.capitalize()}"):
            if marketplace == "ifood":
                if not client_id or not client_secret or not merchant_id:
                    st.warning("Preencha todas as credenciais na barra lateral primeiro.")
                else:
                ...
                st.warning(f"A atualização de PDVs para {marketplace} ainda não foi implementada.")
...
with tab4:
    ...
    if st.button("🚀 Iniciar Comparação"):
        if marketplace == "ifood":
            ...
            if not all([client_id, client_secret, merchant_id, v_instancia, v_id_unidade, v_login, v_senha]):
                ...
                with st.spinner("Realizando comparação..."):
                    try:
                        ...
                        session, url_login = logar_vuca(v_login, v_senha, v_instancia, v_id_unidade, marketplace)
                        itens_vuca = extrair_cardapio_vuca(session, url_login, v_id_unidade, marketplace)
                        df_vuca = pd.DataFrame(itens_vuca)
                        ...
                        st.download_button(
                            ...
                            file_name=f"Comparacao_{marketplace}_{merchant_id}_{data_str}.xlsx",
                            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                        )
            ...
            st.warning(f"A comparação de cardápios para {marketplace} ainda não foi implementada.")